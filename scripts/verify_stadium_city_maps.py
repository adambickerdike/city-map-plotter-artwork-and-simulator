...
from city_map_plotter.production_header import verify_city_header
...
def verify(package=PACKAGE):
    ...
    for row in rows:
        ...
        old = m["stadium_composition"]["old_extent_wgs84"]
        if (extent["east"] - extent["west"]) / (old["east"] - old["west"]) < 1.14999:
            raise ValueError("Map not zoomed out")
        if not (
            extent["west"] < old["west"]
            and extent["east"] > old["east"]
            and extent["south"] < old["south"]
            and extent["north"] > old["north"]
        ):
            raise ValueError("Original map extent cropped")
