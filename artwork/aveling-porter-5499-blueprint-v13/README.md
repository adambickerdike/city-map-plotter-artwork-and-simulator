# Aveling & Porter No. 5499 - custom blueprint, revision 13
...
## Connected quadrant lever

The control lever now continues **below the toothed quadrant**, on the same
straight axis as the handle above. Its two lower edges pass behind the rear
wheel and into the existing chassis, as shown in the retained actual-engine
photograph. The lower pivot and internal linkage are obscured in that view
and are not invented as visible hardware.

The extension meets the quadrant cleanly and is clipped by the wheel and
frame. The export audit confirms that this lower blade is the **only added
geometry**: every previously accepted path is unchanged from revision 12.
The larger plaque remains left of the horse, the firebox keeps its rounded
upright, and the roller keeps its lowered position.
...
|---|---|---:|
| White 0.30 mm | Fine mechanics, worksplate and small footer lettering, inner frame and rules | 709 |
...
The exported master contains **622 engine paths and 840 total strokes**.
...
- Every previous engine, badge, lettering and frame path matches revision 12 exactly. Complete font-stroke and exact pen-width checks pass.
...
```bash
git lfs pull --include="artwork/aveling-porter-5499-blueprint-v13/**"
```
...
Revision 12 remains available as the previous published edition. Earlier working revisions remain local archives.
