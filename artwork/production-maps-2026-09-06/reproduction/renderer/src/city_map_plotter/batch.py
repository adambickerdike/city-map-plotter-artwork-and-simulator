...
def _planned_bbox(
    ...
    *,
    ...
    if preset_format_id is not None:
        ...
        plate_format = load_plate_format(preset_format_id)
        aspect_key = (
            "city_map_field_aspect"
            if _argv_value(argv, "--poster-layout") == "city-map"
            else "map_field_aspect"
        )
        map_field_aspect = float(plate_format[aspect_key])
        return (
...
def _planned_layout(
    ...
    *,
    ...
    if poster_format_id is not None:
        return make_poster_layout(
            bbox,
            preset=preset,
            format_id=poster_format_id,
            poster_layout=_argv_value(argv, "--poster-layout") or "classic",
        )
...
def _artifact_contract(
    *,
    ...
    if subject_copy is not None:
        ...
        detail_lines.extend((purpose,))
        if subject.is_city_preview_only:
            detail_lines.append("COURSE NOT INCLUDED")
        detail_lines.append(coordinate)
        if scale_detail and not subject.is_city_preview_only:
    ...
    if poster_layout == "city-map":
        city_map = {
            ...
            "visible_copy": [title, coordinate, "N"],
        }
        # This composition owns its coordinates and deliberately has no
        # purpose, scale or institution detail band.
        detail_lines = []
    ...
    return {
        ...
        **({"city_map": city_map} if city_map is not None else {}),
        "page": {
...
def _svg_matches_contract(
    ...
    poster_layout = rendering.get("poster_layout")
    if poster_layout == "university-memorabilia":
        furniture_matches = {
            "layer-poster_title",
    ...
    elif poster_layout == "city-map":
        furniture_matches = {
            "layer-poster_title",
            "layer-poster_compass",
            "layer-poster_coordinates",
            "layer-poster_border",
        } <= groups and "layer-poster_personalisation" not in groups
    else:
...
def _manifest_matches_contract(
    ...
    source_cohort = contract.get("source_cohort")
    design_contract = contract.get("design_contract")
    city_map = contract.get("city_map")
    ...
    city_map_matches = (
        _mapping_has_values(manifest.get("city_map"), city_map)
        if isinstance(city_map, dict)
        else manifest.get("city_map") is None
    )
    return (
