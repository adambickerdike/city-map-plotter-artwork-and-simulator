...
class HighwayCompletenessRecord:
    ...
    label_knockout_evidence_ids: tuple[str, ...] = ()
    ...
    def as_dict(self) -> dict[str, Any]:
        ...
        value["label_knockout_evidence_ids"] = list(self.label_knockout_evidence_ids)
        return value
...
class HighwayCompletenessAudit:
    ...
    label_knockout_evidence_supplied: bool = False
    label_knockout_evidence_ledger_valid: bool = False
    label_knockout_evidence_supplied_count: int = 0
    label_knockout_evidence_valid_count: int = 0
    label_knockout_hidden_source_refs: tuple[str, ...] = ()
    invalid_label_knockout_evidence: tuple[tuple[str, str], ...] = ()
    ...
    def as_dict(self, *, include_records: bool = True) -> dict[str, Any]:
        ...
        result: dict[str, Any] = {
            ...
            "course_label_knockout_evidence": {
                "schema_version": 1,
                "policy": "course-label-knockout-v1",
                "ledger_supplied": self.label_knockout_evidence_supplied,
                "ledger_valid": self.label_knockout_evidence_ledger_valid,
                "supplied_entry_count": self.label_knockout_evidence_supplied_count,
                "valid_entry_count": self.label_knockout_evidence_valid_count,
                "invalid_entry_count": len(self.invalid_label_knockout_evidence),
                "hidden_source_ref_count": len(
                    self.label_knockout_hidden_source_refs
                ),
                "hidden_source_refs": list(self.label_knockout_hidden_source_refs),
                "invalid_entries": [
                    {"entry": entry, "reason": reason}
                    for entry, reason in self.invalid_label_knockout_evidence
                ],
            },
            "ink_budget_policy": {
                "schema_version": 2,
...
class RawGeometryIntegrityFinding:
    """One machine-readable finding in selected raw source geometry."""
...
def _relation_ring_role(reference: str) -> str | None:
    """Return the explicit area role from a canonical relation-ring reference."""

    parsed = _parse_source_ref(reference)
    if parsed is None or parsed[0] != "relation":
        return None
    role, separator, index = parsed[2].partition(":ring-")
    if separator and role in {"outer", "inner"} and index.isdigit():
        return role
    return None
...
def _feature_geometry_index(
    ...
    for feature in features:
        ...
        reference_ring_role = _relation_ring_role(serialized)
        if (
            reference_ring_role is not None
            and feature.ring_role != reference_ring_role
        ):
            # A relation-ring part name is itself role-bearing.  Do not let a
            # malformed canonical feature claim an inner source reference with
            # outer-ring semantics (or vice versa).
            continue
...
def _raw_identified_heritage_site_is_polygon(element: dict[str, Any]) -> bool:
    ...
    if osm_type == "relation":
        return _tags(element).get("type", "").strip().casefold() == "multipolygon"
    if osm_type != "way":
        return False
    ...
    if len(parts) != 1:
        return False
    points = parts[0]
    if len(points) < 4 or tuple(round(value, 7) for value in points[0]) != tuple(
        round(value, 7) for value in points[-1]
    ):
        return False
    return (
        abs(
...
def audit_raw_geometry_integrity(
    ...
    *inside* one member's geometry is not.  For intact source objects, every
    ...
    for element_index, value in enumerate(elements_value):
        ...
        failure_count_before = sum(finding.severity == "error" for finding in findings)
                ...
                for member_index, member in enumerate(members):
                    ...
                    if member_type == "node":
                        if not _valid_osm_identifier(member_ref_value):
                            ...
                            continue
                        if layer in AREA_LAYERS and raw_role in {"outer", "inner"}:
                            # Some real multipolygons carry explicitly role-bearing
                            # nodes alongside their drawable way rings (for example,
                            # OSM relation 3227836).  A point cannot contribute line
                            # geometry to a polygon ring, so preserve and audit the
                            # way geometry while recording the ignored node as an
                            # advisory.  A blank role remains ambiguous and fatal.
                            add_finding(
                                reason="ignored_node_member_in_area_geometry_role",
                                layer=layer,
                                source_ref=source_ref,
                                element_index=element_index,
                                osm_type=osm_type,
        ...
        failure_count_after = sum(finding.severity == "error" for finding in findings)
        if failure_count_after != failure_count_before or not identity_valid:
            continue
        ...
        for reference, signature in requirements.items():
            ...
            if signature in canonical.get(reference, set()):
                continue
            ring_role = _relation_ring_role(reference)
            reindexed_matches = (
                sorted(
                    candidate_reference
                    for candidate_reference, candidate_signatures in canonical.items()
                    if candidate_reference != reference
                    and _relation_ring_role(candidate_reference) == ring_role
                    and signature in candidate_signatures
                )
                if ring_role is not None
                else []
            )
            if reindexed_matches:
                # Canonical extraction deliberately de-duplicates an exact
                # same-role relation ring.  That can shift subsequent ring
                # indexes, so source-reference equality alone would report a
                # geometry loss even though the exact geometry survives.  Do
                # not relax geometry or role identity: a unique dropped ring,
                # changed coordinates, or an outer/inner mismatch still falls
                # through to the fatal finding below.
                add_finding(
                    reason="exact_same_role_relation_ring_reindexed",
                    layer=layer,
                    source_ref=source_ref,
                    element_index=element_index,
                    osm_type=osm_type,
                    osm_id=osm_id,
                    component=f"{reference}->{reindexed_matches[0]}",
                    member_role=ring_role,
                    severity="warning",
                )
                continue
            add_finding(
...
@dataclass(frozen=True)
class _LabelKnockoutEvidenceAudit:
    supplied: bool
    ledger_valid: bool
    supplied_count: int
    valid_count: int
    hidden_refs_by_layer: dict[str, frozenset[str]]
    hidden_source_refs: tuple[str, ...]
    invalid_entries: tuple[tuple[str, str], ...]


_LABEL_KNOCKOUT_REMOVAL_REASON = "fully-removed-by-course-label-knockout"


def _audit_label_knockout_evidence(evidence: Any) -> _LabelKnockoutEvidenceAudit:
    """Independently verify the course-label knockout removal ledger.

    The knockout runs after physical compilation, so a source way whose last
    drawn stroke sat wholly under a label vanishes only at the SVG stage.  The
    ledger supplied by ``knock_out_labels`` is the rendering pipeline's claim
    that each such absence was a deliberate, counted design decision; this
    audit checks the ledger's shape and its own bookkeeping before any record
    is allowed to cite it instead of ``svg_lineage_or_serialization_drop``.
    """

    empty = _LabelKnockoutEvidenceAudit(
        supplied=False,
        ledger_valid=False,
        supplied_count=0,
        valid_count=0,
        hidden_refs_by_layer={},
        hidden_source_refs=(),
        invalid_entries=(),
    )
    if not isinstance(evidence, dict) or not evidence.get("applied"):
        return empty
    entries = evidence.get("fully_removed")
    if not isinstance(entries, list):
        return _LabelKnockoutEvidenceAudit(
            supplied=True,
            ledger_valid=False,
            supplied_count=0,
            valid_count=0,
            hidden_refs_by_layer={},
            hidden_source_refs=(),
            invalid_entries=(("fully_removed", "not-a-list"),),
        )
    invalid: list[tuple[str, str]] = []
    hidden_by_layer: dict[str, set[str]] = defaultdict(set)
    hidden_entry_count = 0
    valid_count = 0
    for index, entry in enumerate(entries):
        entry_id = f"entry-{index}"
        if not isinstance(entry, dict):
            invalid.append((entry_id, "not-a-mapping"))
            continue
        layer = entry.get("layer")
        part = entry.get("part")
        refs = entry.get("source_refs")
        length_mm = entry.get("length_mm")
        last_representation = entry.get("last_source_representation")
        if not isinstance(layer, str) or not layer:
            invalid.append((entry_id, "missing-layer"))
            continue
        if not isinstance(part, str) or not part:
            invalid.append((entry_id, "missing-part"))
            continue
        if not isinstance(refs, list) or not all(
            isinstance(ref, str) and "/" in ref for ref in refs
        ):
            invalid.append((entry_id, "malformed-source-refs"))
            continue
        if not isinstance(length_mm, (int, float)) or length_mm < 0:
            invalid.append((entry_id, "invalid-length"))
            continue
        if entry.get("reason") != _LABEL_KNOCKOUT_REMOVAL_REASON:
            invalid.append((entry_id, "unexpected-removal-reason"))
            continue
        if not isinstance(last_representation, bool):
            invalid.append((entry_id, "missing-last-representation-flag"))
            continue
        valid_count += 1
        if last_representation:
            hidden_entry_count += 1
            for ref in refs:
                hidden_by_layer[layer].add(ref)
    if evidence.get("strokes_fully_removed") != len(entries):
        invalid.append(("strokes_fully_removed", "count-mismatch"))
    if evidence.get("source_ways_fully_hidden") != hidden_entry_count:
        invalid.append(("source_ways_fully_hidden", "count-mismatch"))
    ledger_valid = not invalid
    return _LabelKnockoutEvidenceAudit(
        supplied=True,
        ledger_valid=ledger_valid,
        supplied_count=len(entries),
        valid_count=valid_count,
        hidden_refs_by_layer=(
            {layer: frozenset(refs) for layer, refs in hidden_by_layer.items()}
            if ledger_valid
            else {}
        ),
        hidden_source_refs=tuple(
            sorted({ref for refs in hidden_by_layer.values() for ref in refs})
        )
        if ledger_valid
        else (),
        invalid_entries=tuple(invalid),
...
def audit_highway_completeness(
    ...
    *,
    ...
    label_knockout_omission_evidence: Any = None,
    svg: Path | ET.Element | ET.ElementTree | None = None,
    detail_profile: str = "faithful",
    source_query: str | None = None,
    include_disabled_layers: bool = True,
    ...
    ``include_disabled_layers=False`` retains that broad pinned acquisition but
    omits per-object lineage records for classes this plate cannot draw.  It is
    a performance scope, not a source mutation; enabled layers still receive the
    complete raw-to-SVG audit.
    """
    ...
    label_knockout_evidence_audit = _audit_label_knockout_evidence(
        label_knockout_omission_evidence
    )
    ...
    if not isinstance(elements_value, list):
        return HighwayCompletenessAudit(
            ...
            label_knockout_evidence_supplied=(
                label_knockout_evidence_audit.supplied
            ),
            label_knockout_evidence_ledger_valid=(
                label_knockout_evidence_audit.ledger_valid
            ),
            label_knockout_evidence_supplied_count=(
                label_knockout_evidence_audit.supplied_count
            ),
            label_knockout_evidence_valid_count=(
                label_knockout_evidence_audit.valid_count
            ),
            label_knockout_hidden_source_refs=(
                label_knockout_evidence_audit.hidden_source_refs
            ),
            invalid_label_knockout_evidence=(
                label_knockout_evidence_audit.invalid_entries
            ),
        )
    ...
    for index, value in enumerate(elements_value):
        ...
        if "highway" not in tags and area_highway in FALSE_VALUES:
            continue
        if not include_disabled_layers:
            preliminary_layer = (
                _audited_highway_layer(tags)
                if "highway" in tags
                else "road_areas"
                if area_highway not in FALSE_VALUES
                else None
            )
            if preliminary_layer not in enabled_layers:
                continue
    ...
    for (osm_type, osm_id), raw_elements in sorted(grouped.items()):
        ...
        if expected and not immutable_source_failure and not extracted:
            ...
            # The course-label knockout removes compiled strokes after the
            # physical stage, so a way whose only drawn geometry sat under a
            # label goes missing here and nowhere earlier.  The verified
            # removal ledger is the evidence that the absence is the recorded
            # design decision "label lettering owns its paper", not a
            # serialization fault -- and it is honoured only when it covers
            # every ref the SVG is missing for this way on its expected layer.
            observed_svg_refs = (
                svg_parts.get((osm_type, osm_id, expected_layer), set())
                if expected_layer is not None
                else set()
            )
            required_svg_refs = set(
                visible_required_refs or {f"{osm_type}/{osm_id}"}
            )
            missing_svg_refs = required_svg_refs - set(observed_svg_refs)
            knocked_out_refs = (
                label_knockout_evidence_audit.hidden_refs_by_layer.get(
                    expected_layer, frozenset()
                )
                if expected_layer is not None
                else frozenset()
            )
            if (
                label_knockout_evidence_audit.ledger_valid
                and missing_svg_refs
                and missing_svg_refs <= knocked_out_refs
            ):
                reason = "course_label_knockout"
                label_knockout_evidence_ids = tuple(sorted(missing_svg_refs))
            else:
        ...
        if reason == "course_label_knockout" and label_knockout_evidence_ids:
            notes.append(
                "course_label_knockout_refs="
                + ",".join(label_knockout_evidence_ids)
            )
        notes.append(
        ...
        records.append(
            HighwayCompletenessRecord(
                ...
                label_knockout_evidence_ids=(
                    label_knockout_evidence_ids
                    if reason == "course_label_knockout"
                    else ()
                ),
            )
        )
    ...
    if not include_disabled_layers:
        source_scope_warning += (
            " Per-object lineage is scoped to style-enabled layers; disabled "
            "classes remain in the pinned raw source but cannot emit on this plate."
        )
    return HighwayCompletenessAudit(
        ...
        label_knockout_evidence_supplied=label_knockout_evidence_audit.supplied,
        label_knockout_evidence_ledger_valid=(
            label_knockout_evidence_audit.ledger_valid
        ),
        label_knockout_evidence_supplied_count=(
