...
def validate(svg_path: Path, spec: dict, forced: str | None) -> Report:
    ...
    early_manifest: dict | None = None
    if manifest_path.is_file():
        try:
            candidate = json.loads(manifest_path.read_text(encoding="utf-8"))
            if isinstance(candidate, dict):
                early_manifest = candidate
        except (OSError, json.JSONDecodeError):
            pass
    early_poster_layout = str(
        ((early_manifest or {}).get("rendering") or {}).get("poster_layout", "")
    )
    field = fmt["zones_mm"]["map_field"]
    if early_poster_layout == "city-map":
        city_bands = fmt.get("city_zones_mm") or {}
        field = city_bands.get("city_map_field", field)
    field_box = (
        field["x"],
        field["y"],
        field["x"] + field["width"],
    ...
    budget = dict(fmt.get("ink_budget") or {})
    if budget:
        budget["field_area_mm2"] = field["width"] * field["height"]
        budget["max_ink_mm2"] = (
            budget["field_area_mm2"] * budget["max_coverage"]
        )
    ...
    if not manifest_path.exists():
        ...
        return report
    try:
        manifest = early_manifest or json.loads(
    ...
    city_bands = fmt.get("city_zones_mm") or {}
    if poster_layout == "city-map" and city_bands:
        expected_zones.update(city_bands)
        expected_zones["map_field"] = city_bands["city_map_field"]
