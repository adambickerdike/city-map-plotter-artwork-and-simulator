    ...
    for stroke in strokes:
        ...
        for distance in positions:
            if abs(distance) <= 1e-9:
                ...
                continue
            try:
                shifted_geometry: BaseGeometry = centre.offset_curve(
                    distance,
                    quad_segs=4,
                    join_style="mitre",
            ...
            if not shifted_geometry.is_simple:
                shifted_geometry = unary_union(shifted_geometry)
            for part in _line_parts(shifted_geometry):
