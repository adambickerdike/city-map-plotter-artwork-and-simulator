# Aveling & Porter No. 5499 - custom blueprint, revision 10
...
The accepted single side elevation fills **371.6 × 189.7 mm**, centred above
...
The far steering chain now disappears behind the existing central return pipe
under the boiler. Its hidden portions are removed from the plotted strokes.
The ground line below the rollers is removed. All other engine geometry,
lettering, pen assignments and layout remain identical to revision 9.

The accepted single side elevation, smooth wheel rims and staggered spokes,
steering controls, front chain anchors, corner horse, worksplate and compact
footer are retained. Wheel-perimeter centre guides remain absent.
...
path meets the three-nib length floor. The smallest copy is 2.40 mm on 0.30 mm.
...
[Chain detail comparison](evidence/detail-review.html) ·
...
- Wheel guides and ground datum absent; only far-chain clipping changes from revision 9. Complete font-stroke regeneration and exact pen inventory/width checks remain in force.
...
```bash
git lfs pull --include="artwork/aveling-porter-5499-blueprint-v10/**"
```
...
Earlier working revisions remain local archives; this is the final published edition.
