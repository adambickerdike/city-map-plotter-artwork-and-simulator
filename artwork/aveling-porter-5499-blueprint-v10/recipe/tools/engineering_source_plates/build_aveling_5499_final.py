...
from tools.engineering_source_plates.aveling_5499_v10.lettering import text
from tools.engineering_source_plates.aveling_5499_v10.engine import model
from tools.engineering_source_plates.aveling_5499_v10.badge import add_badge

DATA=Path(__file__).with_name('aveling_5499_v10')
DEFAULT_OUTPUT=ROOT/'examples/technical-objects/aveling-porter-5499-blueprint-v10'
...
def page_context():
    ...
    badge=Rect(drawing.right-10*gap,drawing.y,10*gap,13*gap)
...
def build(output,dpi):
    ...
    m=Affine2D(a=scale,d=scale,e=field.centre[0]-(box[0]+box[2])/2*scale,f=field.centre[1]-(box[1]+box[3])/2*scale)
    ...
    badge_meta=add_badge(smallcopy,outlines,context.zones['heritage_badge'])
    ...
    meta={
      ...
      'layout_override':{'authority':'explicit user request','request':'single large side drawing, compact bottom strip, Invicta horse and enlarged worksplate in clear upper-right space',
      ...
      'drawing_centred_in_field':True,'footer_cells':footer_records,
    ...
    art=PlateArtwork(subject_id=ID,domain='heritage-engine-portrait',subject_kind='steam-road-roller',title='AVELING & PORTER',subtitle='R6 STEAM ROAD ROLLER',
      ...
      credit_line='Engine photographs: Benjamin Matthews; Terry Pinnegar. Drafting: The MERL / Fowler S1021. Invicta: Preston Services. Worksplate: GW Railwayana Auctions.',
      ...
      rendering_preset='heritage-engine-white-blueprint-v10',format_subject_policy='user-authorised-single-elevation-bottom-strip',
