...
def make_poster_layout(
    ...
    *,
    ...
    try:
        ...
        city_zones = {
            name: _rect_from_format(value, name=f"{format_id}.{name}")
            for name, value in resolved.get("city_zones_mm", {}).items()
        }
    ...
    if poster_layout == "city-map":
        try:
            map_zone = city_zones["city_map_field"]
        except KeyError as exc:
            raise MapPlotterError(
                f"Plate format {format_id!r} has no city-only map field."
            ) from exc
        canonical_zones["map_field"] = map_zone
    scale = min(
