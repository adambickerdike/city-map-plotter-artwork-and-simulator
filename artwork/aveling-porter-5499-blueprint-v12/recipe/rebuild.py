...
def main():
    ...
    facts=json.loads((PACKAGE/'evidence/sources.json').read_text())
    names={'revision-11-master.svg','revision-9-master.svg','Rolling_186_Autumn_2021.pdf','reference-landmarks.json'}
    ...
    run('tools/engineering_source_plates/build_aveling_5499_horizontal_badge.py','--output-dir',out)
    ...
    run('tools/plotjob.py','compile',svg,'--profile',profile,'--order','optimised',
        '--out',out/'plot/aveling-porter-5499.plotjob.json')
    run('tools/engineering_source_plates/aveling_5499_v12/verify_drawing.py',out)
    run('tools/build_plotsim_viewer.py',svg,'--out',out/'plot/aveling-porter-5499-viewer.html',
        '--machine-profile',profile,'--strict-svg')
    if not args.skip_pdf:
        ns='http://www.w3.org/2000/svg';ET.register_namespace('',ns)
        tree=ET.parse(svg)
        tree.getroot().insert(0,ET.Element('{'+ns+'}rect',{'width':'420','height':'297','fill':'#113b58'}))
