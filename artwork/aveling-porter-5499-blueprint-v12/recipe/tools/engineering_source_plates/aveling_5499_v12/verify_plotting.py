...
from tools.engineering_source_plates.aveling_5499_v12.badge import curved_copy
...
def audit(package):
    ...
    # Compare the edit with the published sheet. Only the rear-frame outline
    # may change; the rest of the accepted engine, footer and frame must match.
    previous = package / 'evidence/revision-11-master.svg'
    assert hashlib.sha256(previous.read_bytes()).hexdigest() == '84601ac5b686bc1e16fd9b26a042ab9d0e9f972eac3c647877f7a24306c8ac0c'
    ...
    def is_frame_outline(p):
        return p.get('data-component')=='rear-frame-and-platform' and p.get('data-feature') in ['component-outline','continuous-firebox-frame']
    def signature(p):
        return tuple(p.get(k) for k in ['d','data-role','data-copy','data-feature','data-component'])
    def unchanged(p):return not is_badge(p) and not is_frame_outline(p)
    ...
    for name in ['horse','worksplate']:
        ...
        transform=Affine2D(**badge['transforms_from_v11'][name])
    ...
    report = {
        ...
        'maximum_layout_coordinate_errors_mm':layout_errors,'footer_and_frame_unchanged_from_v11':True,
        'all_engine_paths_except_rear_frame_outline_unchanged_from_v11':True,
