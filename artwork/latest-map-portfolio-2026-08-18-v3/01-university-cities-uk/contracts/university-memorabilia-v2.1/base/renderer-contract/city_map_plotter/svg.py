...
POSTER_LAYOUT_CHOICES = frozenset({"classic", "university-memorabilia"})
...
def _layout_plate_format(layout: Layout) -> dict[str, Any]:
...
def _reorder_document_layers_by_pen(
    root: ET.Element, layer_stats: list[dict[str, Any]]
) -> None:
    ...
    ordered = [group for _, key in ordered_keys for group in buckets[key]]
...
def _pen_sequence(
    layer_stats: list[dict[str, Any]], document_layer_ids: list[str]
) -> list[dict[str, Any]]:
    ...
    *,
    ...
    if poster_layout == "university-memorabilia":
        if layout.preset not in A5_POSTER_PRESETS:
            raise MapPlotterError(
                "University memorabilia layout requires an A5 poster preset."
            )
        if include_scale_bar:
            raise MapPlotterError(
                "University memorabilia uses its dedicated header compass and "
                "requires the map-overlay scale bar to be disabled."
            )
    ...
    unexpected_highway_omissions = [
        ...
        if record.reason not in {"physical_minimum_gate", "ink_budget_gate"}
    ]
        ...
        **(
            {
                "memorabilia": {
                    ...
                    "coordinates": coordinate_label(layout),
                    "personalisation": {
