...
def append_memorabilia_head(
    ...
    *,
    ...
    """Serif title, subtitle, coordinates and compass in the head band.

    Shared by every keepsake composition -- the university plate, the course
    plate and the crew plate all wear the same head, so a customer with two
    of them on a wall sees one family.
    """
    ...
    compass_middle = (compass_top + compass_bottom) / 2
    compass_half = compass_zone.width_mm * 0.22
    compass_paths = [
        [
            (compass_x, compass_top),
