    ...
    *,
    ...
    for source, raw_area in sorted(
        ...
        for row in range(first_row, last_row + 1):
            ...
            for column in range(first_column, last_column + 1):
                ...
                if centre_key in seen_centres or not area.covers(Point(x_mm, y_mm)):
                    continue
    ...
    if candidate_dot_count > maximum_dot_count:
        ...
        for index, stroke in enumerate(strokes):
            source_ref = stroke.tags.get("source-refs", "")
            first_index_by_source.setdefault(source_ref, index)
        reserved_indices = sorted(first_index_by_source.values())
...
def _project_feature(
    ...
    if line is None:
        return []
    return list(_line_parts(line.intersection(clip_box)))
...
def prepare_clean_poster_strokes(
    ...
    *,
    ...
    for feature in features:
        parts = _project_feature(feature, layout, map_clip)
        ...
        if layer in AREA_OUTLINE_LAYERS:
            ...
            if layer == "buildings" and landmark_buildings:
                ...
                for coincident_objects in geometry_buckets.values():
                    ...
                    deduplicated_grouped[representative_key] = [
                        (
                            replace(
                                source,
                                tags={
