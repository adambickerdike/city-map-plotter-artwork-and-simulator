...
The ladder is the real studio stock across its named profiles: 0.25 and 0.40 mm in the
general colour set, 0.60 and 1.00 mm in black, 0.70 and 1.00 mm in the default White set,
0.30, 0.40 and 0.50 mm in the separate White blueprint paint-pen set, and 1.00 mm in gold
and silver. The table below
...
## City-only subcomposition

Every format publishes `city_zones_mm`. The city name sits at the top left,
with the coordinates directly underneath on the same ink edge. The compass has
its own column at the right and is centred vertically against the actual title
and coordinate ink bounds, including wrapped titles. Its preferred size derives
from the reviewed A5 compass and is constrained by the header height. The
`city-header-left-stack-v1` contract applies to city and university editions.

Portrait city fields retain their exact crop and extend down to one gap above
the attribution band. University editions share the header while retaining the
default map and personalisation footer. Landscape city formats put the header
across the content width, with the map below. The reviewed A3 landscape field
remains `372 × 217.714 mm` at `(24, 55.286)`.

A3 landscape retains three spaces around the coordinate slash, 0.25 mm tracking
and its 0.25 mm north arrow with a west-east axis. Its arrow geometry remains in
`city_header.compass_geometry`. Other city headers retain the diamond compass.

For A5 portrait, the binding city field is `124 × 155.874 mm` at
`(12, 35.122)`, compared with the default `124 × 128.924 mm` field. A city
edition therefore uses `city_map_field_aspect` when it crops or expands its
geographic extent. It must not stretch coordinates to fill the taller field.
An exact-source derivative uses the same pinned response and a centred `cover`
crop; this enlarges the map while keeping north-up geometry and source lineage.

## Clean personalised university subcomposition

Portrait formats publish `memorabilia_variants.clean-personalised`. It keeps
the default map field and border unchanged while rearranging copy only inside
existing generated zones:

- the university title uses `city_title`, using the binding
  two-line fallback when a single line cannot survive the physical nib gate;
- coordinates use `city_coordinates`, directly below the title, left-aligned on
  one line with three spaces around the slash and one smallest-real-nib of
  tracking between characters; their first ink edge is locked to the title's
  first ink edge rather than merely to a nearby zone boundary;
- the compass uses the dedicated `city_compass` column at the far right,
  centred vertically beside the title and coordinates;
- the name remains in `memorabilia_person_name`; degree spans the otherwise
  unused degree and honours cells, while the year remains right-aligned in
  `memorabilia_years`;
- labels and writing rules are forbidden, and the honours cell must be empty.

Footer values use the bundled Hershey Serif display face on a real 0.40 mm pen.
Name and secondary sizes are derived as 1.70 and 1.50 times the format's
existing `detail` role, and the footer inset is two value-nib widths. A physical
ink-bounds gate prevents any footer stroke from crossing the left or right edge
of `map_field`. The variant therefore owns no private page coordinate and cannot
alter the geographic crop.
