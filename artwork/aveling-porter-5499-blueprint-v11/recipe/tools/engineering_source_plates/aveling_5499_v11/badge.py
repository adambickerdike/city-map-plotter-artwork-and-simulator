...
def add_badge(detail,outline,zone,*,enlargement=1.10):
    """Enlarge geometry and lettering together, retaining the physical pens."""
    base=Rect(0,0,zone.width/enlargement,zone.height/enlargement)
    ...
    transform=Affine2D(a=enlargement,d=enlargement,e=zone.x,f=zone.y)
    for source,target in [(fine,detail),(edges,outline)]:
        for record in source.records:
            attributes=dict(record.attributes)
            for key in ['data-cap-height-mm','data-tracking-mm']:
                if key in attributes:attributes[key]=str(float(attributes[key])*enlargement)
    ...
    centre=transform.apply((plate['cx'],plate['cy']))
    meta.update(zone={'x':zone.x,'y':zone.y,'width':zone.width,'height':zone.height},
                centre_x_mm=zone.centre[0],horse_height_mm=meta['horse_height_mm']*enlargement,
                uniform_enlargement=enlargement,
                worksplate_ellipse_mm={'cx':centre[0],'cy':centre[1],
                    'rx':plate['rx']*enlargement,'ry':plate['ry']*enlargement},
                curved_copy_baseline_mm={'cx':centre[0],'cy':centre[1],
                    'rx':(plate['rx']-3.2)*enlargement,'ry':(plate['ry']-5.2)*enlargement},
                previous_zone_mm={'x':zone.right-base.width,'y':zone.y,
