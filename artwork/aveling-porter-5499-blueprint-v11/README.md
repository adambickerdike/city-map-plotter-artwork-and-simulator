# Aveling & Porter No. 5499 - custom blueprint, revision 11
...
The complete roller is moved **6 mm down** on the A3 sheet, retaining its exact
size, proportions and linework. The separate horse and worksplate are enlarged
**10% together**, including their lettering and spacing. The group remains
anchored at the upper right. Physical pen widths are retained, not enlarged.

The footer and frame remain in place. The far chain stays behind the central
return pipe, and the ground line and wheel-perimeter guides remain absent.
The exported geometry is checked against the published revision 10 master:
only the requested engine translation and badge enlargement are allowed.
...
path meets the three-nib length floor. The smallest copy is 2.64 mm on 0.30 mm.
...
| White 0.40 mm | Main engine contours, emblem outline and larger footer lettering | 129 |
...
The exported master contains **623 engine paths and 841 total strokes**.
All 116 stock-format checks and strict SVG preflight pass. The checks cover:

- 706 open engine-contour ends, with no unexplained gaps; the cotter's free tips are intentional.
...
- Exact 6 mm engine translation and 10% badge enlargement from revision 10; footer and frame unchanged. Badge-to-control clearance, complete font-stroke regeneration and exact pen inventory/width checks pass.
...
```bash
git lfs pull --include="artwork/aveling-porter-5499-blueprint-v11/**"
```
...
Revision 10 remains available as the previous published edition. Earlier working revisions remain local archives.
