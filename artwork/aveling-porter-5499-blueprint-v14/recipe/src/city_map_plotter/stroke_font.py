...
STROKE_FONT_CONTRACT_SCHEMA_VERSION = 4
STROKE_FONT_ID = "plotter-grid-v4"
TEXT_NORMALISATION_POLICY_ID = "nfc-supported-diacritics-v2"
ASCII_TRANSLITERATION_POLICY_ID = "nfkd-ascii-transliteration-v2"
...
def _marked_glyph(base: str, *marks: Stroke) -> list[Stroke]:
    """Compose a pen-safe precomposed glyph from one base and accent strokes."""

    return [[*stroke] for stroke in GLYPHS[base]] + [[*mark] for mark in marks]


_ACUTE: Stroke = [(0.7, -0.15), (3.4, -1.3)]
_GRAVE: Stroke = [(0.6, -1.3), (3.3, -0.15)]
_CIRCUMFLEX: Stroke = [(0.2, -0.15), (2.0, -1.3), (3.8, -0.15)]
_TILDE: Stroke = [(0.1, -0.35), (1.1, -1.05), (2.9, -0.35), (3.9, -1.05)]
_DIAERESIS_LEFT: Stroke = [
    (0.3, -0.5),
    (0.7, -0.9),
    (1.1, -0.5),
    (0.7, -0.1),
...
for _character, _base, _marks in (
    ("Á", "A", (_ACUTE,)),
    ("É", "E", (_ACUTE,)),
    ("È", "E", (_GRAVE,)),
    ("Í", "I", (_ACUTE,)),
    ...
    ("Ô", "O", (_CIRCUMFLEX,)),
    ("Ñ", "N", (_TILDE,)),
    ("Ö", "O", (_DIAERESIS_LEFT, _DIAERESIS_RIGHT)),
    ("Ü", "U", (_DIAERESIS_LEFT, _DIAERESIS_RIGHT)),
    ("á", "a", (_ACUTE,)),
    ("é", "e", (_ACUTE,)),
    ("è", "e", (_GRAVE,)),
    ("í", "i", (_ACUTE,)),
    ...
    GLYPHS[_character] = _marked_glyph(_base, *_marks)

GLYPHS["Þ"] = [
    [(0, 6), (0, 0)],
    [(0, 1.25), (3, 1.25), (4, 2.25), (4, 3.75), (3, 4.75), (0, 4.75)],
]
GLYPHS["þ"] = [
    [(0, 6), (0, 0)],
    [(0, 2), (3, 2), (4, 3), (4, 4.5), (3, 5.5), (0, 5.5)],
]
...
_ASCII_TRANSLITERATION_MAP: dict[str, str] = {"Þ": "TH", "þ": "th"}
_ASCII_TRANSLITERATIONS: dict[int, str] = {
    ord(character): replacement
    for character, replacement in _ASCII_TRANSLITERATION_MAP.items()
}


def _ascii_transliteration_sha256() -> str:
    payload = {
        "policy_id": ASCII_TRANSLITERATION_POLICY_ID,
        "unicode_form": "NFKD",
        "combining_mark_policy": "drop",
        "replacements": sorted(_TEXT_REPLACEMENT_MAP.items()),
        "ascii_transliterations": sorted(_ASCII_TRANSLITERATION_MAP.items()),
    }
...
ASCII_TRANSLITERATION_SHA256 = _ascii_transliteration_sha256()
...
def _stroke_font_data_sha256(
    ...
    ascii_transliterations: Mapping[str, str] | None = None,
) -> str:
    ...
    ascii_transliterations = (
        _ASCII_TRANSLITERATION_MAP
        if ascii_transliterations is None
        else ascii_transliterations
    )
    ...
    if any(len(character) != 1 for character in ascii_transliterations):
        raise ValueError("ASCII transliteration keys must be single characters.")
    if any(not isinstance(value, str) for value in ascii_transliterations.values()):
        raise ValueError("ASCII transliteration values must be strings.")
    ...
    payload = {
        ...
        "normalisation": {
            ...
            "unicode_form": "NFC",
            "combining_mark_policy": "preserve supported precomposed glyphs; "
            "NFKD fallback for unsupported letters",
            "replacements": [
            ...
            "ascii_transliterations": [
                {
                    "codepoint": ord(character),
                    "replacement": ascii_transliterations[character],
                }
                for character in sorted(ascii_transliterations, key=ord)
            ],
        },
        "metrics": {
...
def normalise_text(text: str) -> str:
    """Preserve supported diacritics and transliterate unsupported Latin letters."""

    composed = normalize("NFC", text.translate(_TEXT_REPLACEMENTS))
    result: list[str] = []
    for character in composed:
        if character == " " or character in GLYPHS:
            result.append(character)
            continue
        decomposed = normalize("NFKD", character)
        fallback = "".join(item for item in decomposed if not combining(item))
        result.append(fallback.translate(_ASCII_TRANSLITERATIONS))
    return "".join(result)


def transliterate_text(text: str) -> str:
    """Return the former ASCII-only fallback for fonts without diacritics."""

    decomposed = normalize("NFKD", text.translate(_TEXT_REPLACEMENTS))
    plain = "".join(character for character in decomposed if not combining(character))
    return plain.translate(_ASCII_TRANSLITERATIONS)
...
def text_width_mm(
    ...
    *,
    ...
    """Width of ``text`` with optional physical tracking, in millimetres."""

    if (
        isinstance(tracking_mm, bool)
        or not isinstance(tracking_mm, (int, float))
        or not isfinite(tracking_mm)
        or tracking_mm < 0
    ):
        raise MapPlotterError("Text tracking must be a finite non-negative number.")
    drawable = _drawable_text(text)
    return (
        _text_width_units(drawable) * cap_height_mm / FONT_CELL_HEIGHT_UNITS
        + max(len(drawable) - 1, 0) * tracking_mm
    )
...
def stroke_text(
    ...
    *,
    ...
    width_mm = (
        _text_width_units(drawable) * scale
        + max(len(drawable) - 1, 0) * tracking_mm
    )
    ...
    for character in drawable:
        if previous:
            cursor += kern_units(previous, character) * scale + tracking_mm
        previous = character
        if character == " ":
