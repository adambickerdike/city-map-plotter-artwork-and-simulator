...
# Runaway-download guard. 128 MiB is ample for a city-centre crop, but a
# marathon frames its own extent -- Berlin's course spans 10 km, Boston's 36 --
# and an all-roads extract at that size legitimately exceeds it. Raising the
# ceiling is therefore an explicit operator choice (`--max-response-mb`), never
# a silent one, so an accidental continent-sized query still stops here.
_MAX_RESPONSE_BYTES = 128 * 1024 * 1024


def _response_limit_bytes(limit_mb: float | None) -> int:
    if limit_mb is None:
        return _MAX_RESPONSE_BYTES
    if not limit_mb > 0:
        raise MapPlotterError("The map service response limit must be positive.")
...
def build_overpass_query(
    ...
    *,
    ...
    landmark_green_space_only: bool = False,
    landmark_refs: tuple[str, ...] = (),
    road_highway_values: frozenset[str] | None = None,
    linear_water_only: bool = False,
    major_river_context_only: bool = False,
) -> str:
    ...
    if "roads" in families:
        ...
        # clipping.
        if road_highway_values is None:
            statements.append(f'way["highway"]({box});')
        elif road_highway_values:
            statements.append(
                f'way["highway"~"{_value_regex(road_highway_values)}"]({box});'
            )
    ...
    if "water" in families:
        if major_river_context_only:
            # A metropolitan overview still needs its defining river, but not
            # every drain, ditch, pond, wetland, or reservoir.  Tidal rivers
            # such as the Thames are commonly represented by a water-surface
            # multipolygon rather than by one useful centreline.  Select the
            # narrow river vocabulary explicitly so the surface survives the
            # scale filter without reintroducing an unbounded all-water query.
            statements.extend(
                (
                    f'way["waterway"="river"]({box});',
                    f'way["waterway"="riverbank"]({box});',
                    f'relation["waterway"="riverbank"]({box});',
                    f'way["natural"="water"]["water"="river"]({box});',
                    f'relation["natural"="water"]["water"="river"]({box});',
                )
            )
        else:
            statements.append(
        ...
        if not linear_water_only and not major_river_context_only:
            statements.extend(
                (
    ...
    if "parks" in families:
        if landmark_green_space_only:
            for object_type in ("way", "relation"):
                statements.extend(
                    (
                        f'{object_type}["leisure"~"^(nature_reserve|park|golf_course)$"]["name"]({box});',
                        f'{object_type}["landuse"="forest"]["name"]({box});',
                        f'{object_type}["natural"~"^(heath|wood)$"]["name"]({box});',
                    )
                )
        else:
            statements.extend(
...
def _decode_response_payload(
    payload: bytes, content_encoding: str, limit_bytes: int = _MAX_RESPONSE_BYTES
) -> bytes:
    encodings = {
    ...
    if "gzip" not in encodings and "x-gzip" not in encodings:
        if len(payload) > limit_bytes:
            raise MapPlotterError(
                f"Map service response exceeds the {limit_bytes // (1024 * 1024)} MiB "
                "safety limit. Raise --max-response-mb if this extent is intended."
            )
        return payload
    try:
        with gzip.GzipFile(fileobj=io.BytesIO(payload)) as stream:
            decoded = stream.read(limit_bytes + 1)
    except (EOFError, OSError) as exc:
        raise MapPlotterError("Map service returned invalid gzip data.") from exc
    if len(decoded) > limit_bytes:
        raise MapPlotterError(
            f"Decompressed map service response exceeds the "
            f"{limit_bytes // (1024 * 1024)} MiB safety limit. Raise "
            "--max-response-mb if this extent is intended."
        )
    return decoded


def _http_error_detail(exc: urllib.error.HTTPError) -> str:
    """A human-readable extract of an HTTP error body.

    Error responses honour the request's ``Accept-Encoding: gzip``, so the body
    must be decompressed before decoding or the message is mojibake.  A gateway
    error page is often HTML; the tags are stripped so the operator sees the
    ...
    try:
        ...
        return ""
    encoding = (exc.headers.get("Content-Encoding") or "").casefold()
    if "gzip" in encoding:
        try:
            with gzip.GzipFile(fileobj=io.BytesIO(payload)) as stream:
...
def _request_json(
    ...
    *,
    ...
    limit_bytes: int = _MAX_RESPONSE_BYTES,
    ...
    for attempt in range(transient_retries + 1):
        ...
        payload = _decode_response_payload(payload, content_encoding, limit_bytes)
        try:
...
def fetch_overpass(
    ...
    *,
    ...
    query = build_overpass_query(
        ...
        landmark_green_space_only=landmark_green_space_only,
        landmark_refs=canonical_refs,
        road_highway_values=road_highway_values,
        linear_water_only=linear_water_only,
    ...
    source_metadata = {
        ...
        "landmark_green_space_only": landmark_green_space_only,
        "road_highway_values": (
            sorted(road_highway_values) if road_highway_values is not None else None
        ),
        "linear_water_only": linear_water_only,
        "major_river_context_only": major_river_context_only,
    }
    ...
    result = _request_json(
        ...
        data=body,
        transient_retries=2,
        limit_bytes=_response_limit_bytes(max_response_mb),
    )
...
def fetch_course_relation(
    ...
    *,
    ...
    query = build_course_relation_query(
        relation_id, timeout_s=_overpass_server_timeout(timeout_s)
    )
    cache_path = cache_dir / "overpass" / f"{_cache_key(endpoint, query)}.json.gz"
    if cache_path.exists() and not refresh:
        cached = _validate_overpass(_load_json(cache_path), str(cache_path))
